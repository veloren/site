...
{{ pgitlab(ids=["!4627"]) }} (fixes and follow-ups: {{ gitlab(ids=["!4649", "!4641", "!4639"]) }})
...
- Persisting server authoritative physics {{ gitlab(ids=["!4211"]) }}
- Expose the duration of bans to clients {{ gitlab(ids=["!4616"]) }}
...
- Improvements to sprite models, by @uniior and @Supa {{ gitlab(ids=["!4635"]) }}
- Ensuring every house has a bed, by @fotkurz {{ gitlab(ids=["!4609"]) }}
- Riposte and defensive stance balance, by @necti {{ gitlab(ids=["!4634"]) }} and @do-no-van {{ gitlab(ids=["!4526"]) }}
- Making sceptres use the default equip/unequip SFX, by @horblegorble {{ gitlab(ids=["!4628"]) }}
- Make Iron Tempest (Hammer ability) multi-target, by @necti {{ gitlab(ids=["!4633"]) }}
- Adding missing changelog entries, by @horblegorble {{ gitlab(ids=["!4624", "!4615"]) }}
- Various cheesing fixes, by @floppy {{ gitlab(ids=["!4621", "!4620"]) }}
- Adding a deletion timeout for all sprite summon abilities, by @horblegorble {{ gitlab(ids=["!4630"]) }}
- Fixing the mod badge location in group and faction chats, by @Hgrebnednav {{ gitlab(ids=["!4636"]) }}
- Removing the code used to render old dungeons, by @floppy {{ gitlab(ids=["!4613"]) }}
...
@Tarhun is working on reducing the chance for towns to generate without any plots. {{ gitlab(ids=["!4646"]) }}
