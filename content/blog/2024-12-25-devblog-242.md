...
{{ pgitlab(ids=["!164"], project="airshipper") }}
...
- Fixing a typo and the flurry ability icon not showing when not in agility stance, by @Vaga {{ gitlab(ids=["!4670"]) }}
- Legoom sounds, by @GeekyGami and @DaforLynx {{ gitlab(ids=["!4655"]) }}
- Fixing the hammer/axe block animation, by @miocore {{ gitlab(ids=["!4669"]) }}
- Updating `rustls` and `idna` to address RUSTSEC advisories, by @crab {{ gitlab(ids=["!4668"]) }}
- Updating dependencies, by @NeuralModder {{ gitlab(ids=["!4653"]) }}
- Fix RTsim NPCs dying permanently, by @crab {{ gitlab(ids=["!4650"]) }}
...
- Respawn rework, by @isse {{ gitlab(ids=["!46"], project="rfcs") }}
- **DRAFT**: Removing/Deprecating the general combat skill tree, by @DaforLynx {{ gitlab(ids=["!45"], project="rfcs") }}
