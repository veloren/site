...
{{ pgitlab(ids=["!160"], project="airshipper") }}
...
{{ pgitlab(ids=["!162"], project="airshipper") }}
...
- Cheesing fixes for the Vampire castle, by @floppy {{ gitlab(ids=["!4588"]) }}
- Dependency upgrades, by @xMAC {{ gitlab(ids=["!4580", "!4582"]) }}
- Taiwanese translation work, by @skygrango {{ gitlab(ids=["!4592"]) }}
- Fixing errors in translation files, by @Thibagon and @fnetX {{ gitlab(ids=["!4528", "!4576"]) }}
- Fixing server crashes caused by the `/buff` command, by @crab {{ gitlab(ids=["!4594"]) }}
- Updating tracy-client to the latest version, by @imbris {{ gitlab(ids=["!4578"]) }}
- Fixing how buff strength is displayed, by @horblegorble {{ gitlab(ids=["!4574"]) }}
- Updating the contribution guidelines, by @isse {{ gitlab(ids=["!4529"]) }}
- Improvements to the sprite manifest, by @imbris {{ gitlab(ids=["!4577"]) }}
- WGPU fixes and re-enabling shaderc optimizations, by @imbris {{ gitlab(ids=["!4579"]) }}  
...
@juliancoffee is working on adding `LootSpec` to `SpriteCfg`. This will allow specifying separate loot tables for the same sprite kind in worldgen, making it easier to handle keys or other special drops in dungeons without creating more sprites {{ gitlab(ids=["!4354"]) }}

@do-no-van is reworking throwables, they can now be equipped and used like other weapons! {{ gitlab(ids=["!4593"]) }}
