...
- [The State of Graphics and UI by @Imbris](#the-state-of-graphics-and-ui-by--imbris)
...
- [Why Veloren is Special (to me) by @Christof](#why-veloren-is-special--to-me--by--christof)
...
- [Looking to the Graphical Future by @Sharp](#looking-to-the-graphical-future-by--sharp)
...
gems. When the new model-skeleton for smaller bipeds was introduced however, I
...
Unfortunately, this feature didn't work as intended for a further two months as I
...
the inventory itself will actually be "inside" the equipped bag item that provides
that slot.
...
to quickly drop a specific bag of stuff on the ground in order to run faster and
escape imminent death.
...
each chunk is stored in the terrain metadata. Querying that information was
...
regen. Most of the core devs do not like the idea of a passive health regen for
...
enough to
slow down everything. I was even happier to actually see 100 people on our
lastest party. In its current state, none of the devs could dream of it scaling so well. Anyway, I owe you some
...
hitboxes. We break it up into 2 parts: first, we do entity <> entity collisions.
...
In order to speed this process up we do a quick-check where we approximately some
parameters, so quickly skip far apart entities, though this check is far from
...
computations for leaves gen. You can’t see it on the graph but on the 5th iteration, we are at 7ms for
...
A while ago I wrote a Discord bot that allows you to control the Veloren
server it manages (which I'm able to pay for with the help of my patrons - thank
...
Rust, which was the perfect candidate for me to see how the language was
practical. The community was very welcoming and I got up to speed to do my
