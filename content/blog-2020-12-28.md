...
required a more or less fully automated way to generate a layout so the
developer (Sam) writing the underlying systems just has to place simple button
...
a bit of CSS and LUA when I joined the Veloren Team in 2018 with my last CS
class dating back to around 2008 when I started to learn graphic design. My
origins are actually the World of Warcraft UI modding community, which you may
or may not recognize in some of the Veloren UI elements ;)

Whoever is looking for a beginner (or probably slightly advanced beginner...)
language: Try out Rust.

~Pfau

# Slipped

i'd say so yep, we've been asking people to test anyways, so it's not like it's
too WIP to show off or anything so the basic idea: I'm working on animating a
branch by lboklin that adds realistic glider physics to the game. While the
...
and hanging freely when coasting. that's about the extent of the detail i can do
before is start butchering it :smile:
...
content. I hope this will do! I'm a bit of a dry well at the moment
...
I started to contribute to Veloren by creating some npcs models. Learning how to
make them has been a really good learning experience as that was the first time
I played with voxel modeling. After delving into the process to add these new
species into the game, I have discovered how to make skeletons and animations
...
ecosystem. There are now almost 100 different species and we are only starting
...
simple features like a wepon speed stat or allowing items to have buffs. For the
...
**"Has the server crashed?"** Anyone who spent more than a few days in #general
...
inventory/loadout handling. For as long as I've been part of the project players
...
loadout. Not only will this allow players to carry many more items than today,
...
that slot. This functionality won't have much of an impact when it's first
...
have helped me a lot. My first major contribution was adding additional skills
...
was quite overwhelming initially. Since then, I have had fun programming
...
concerned). Minimizing the difference between players and NPCs is part of our
...
split-second decision making. Currently, humanoid NPC attacks tend to be a
...
it). I also got to work with slipped on some animal attacks. Currently testing
...
RON files. After working with combat for a while, I ventured into the land of
...
before we decided to let the songs play to their end. Biomes, as defined in the
...
These are used for falling leaves, beehives, fireflies, and fireplaces. I
repurposed the system to emit sound effects from blocks. The sound effects can
...
the future this will likely be tied to weather simulation. As more people play
...
DPS allowing for more play styles. Armor also will have a poise damage reduction
...
level, the larger the effects on the stunned entity. I am still experimenting
...
I found Veloren at a point when I had a lot of inspiration to write music for
open-world games but I couldn't find where to fulfill it. Finding of Veloren
...
called "Tadpole Tales".

It's more then exciting to be a part of this community , and I'm looking forward
for our future .
...
When i wrote this article it had the working name "how to make sure that the
next release party can handle 100 players". During the investigation i dived
deep into the veloren source code and found multiple regions which do scale very
bad. The server only needs a single bad scaling component which is enough to
slow down everything. I was even more happy to actually see 100 people on our
lastest party. I couldn't dream of it scaling so well. Anyway, i own you some
physics and networking details.

# How we calculate physics:

Physics in this context describes handling positions and velocities and
...
velocity except for mounted entities and projectiles). For each of this entity
we check if it collides with another entity (we skip entities with itself, other
...
For this we iterate over all entities (technially only those with position and
velocity except for mounted entities). We apply movement parameters (gravity,
...
I would like to take this opportunity on this special 100th's blog, to thank all
...
of you is doing his/her unique but essential part for this project!
...
<body class="stackedit">
  <div class="stackedit__html"><p>I wanted to add procedurally generated trees in veloren since the current ones are just prefabs. I’ve so started iterating on tree designs.<br>
The main problem was veloren compile times. (~2mins for each change + 1min to get in-game)<br>
I’ve so designed a wrapper for rust and Java Minecraft plugin and used a custom Minecraft server to lower this time to around 3 seconds. I was so able to iterate really fast.<br>
The firsts ones are as you can see were T shaped:<br>
<img src="https://media.discordapp.net/attachments/449660795857403905/789916495505391636/2020-12-19_19.05.33.png?width=1276&amp;height=676" alt="https://media.discordapp.net/attachments/449660795857403905/789916495505391636/2020-12-19_19.05.33.png?width=1276&amp;height=676"></p>
<p>I’ve so decided to work on a leave grow algorithm and obtained very symmetric trees. This first algorithm was simulating realistic leave growth with params like sun exposure, position, precipitations…<br>
<img src="https://media.discordapp.net/attachments/449660795857403905/790328238474788944/2020-12-20_22.21.12.png?width=1228&amp;height=676" alt="https://media.discordapp.net/attachments/449660795857403905/790328238474788944/2020-12-20_22.21.12.png?width=1228&amp;height=676"><br>
Then Zesterer came to help me and we’ve designed a new branch generation algorithm! After tones of work, iterations, and optimizations we were able to generate very unique, good looking and fun to climb trees! The leave algorithm has been updated to match performance constraints! We’ve so removed all params expected a health param that is different for each branch and based on the soil and sun exposure!<br>
I also rebased the branch gen algorithm on vector projections to reduce block updates and managed to do a 5000% improvement.<br>
<img src="https://media.discordapp.net/attachments/449660795857403905/791320681798959124/screenshot_1608735934388.png?width=1276&amp;height=676" alt="https://media.discordapp.net/attachments/449660795857403905/791320681798959124/screenshot_1608735934388.png?width=1276&amp;height=676"><br>
There is still a lot of optimizations to do so the game can generate trees at chunk load! Zester and I are working on interpolation to reduce the number of computations for leaves gen!</p>
<h3 id="performance-evolution">Performance evolution</h3>
<p><img src="https://i.imgur.com/GUUAqba.png" alt="https://i.imgur.com/GUUAqba.png"><br>
You can’t see it on the graph but on the 5th iteration, we are at 7ms of branches and 130ms for leaves</p>
<h3 id="special-thanks">Special thanks</h3>
<ul>
<li>Zester | Help for branch algorithm and optimization ideas</li>
<li>Sharp, Zester | Code reviewing</li>
<li>Pfau | For the general tree design</li>
<li>Pfau, Gemu, Treeco, Slipped,  James, Sarra_Kitty, Scott, XVar, Snowram | For the tree design corrections</li>
</ul>
</div>
</body>
...
We all can't hide the fact that our website isn't the prettiest one. It works
...
{{
    img(src="https://media.discordapp.net/attachments/590304034175451166/793407297279229962/veloren.net.jpg?width=647&height=1229")
}}
...
special) and a lot more. Suggestions welcome!

And as a side note the green download buttons are not linux exclusive don't
worry. They adjust based on your os automatically.

Furthermore it's fully responsive too:
{{
    img(src="https://media.discordapp.net/attachments/590304034175451166/793407294523179008/veloren.net-mobile.jpg?width=69&height=1227")
}}
...
Some time ago I've written a discord bot which allows you to control the veloren
...
or rather testing of new features so it serves it purpose well.

As it's the 100th edition of the blog a lot of time has passed! When I started I
was a rust beginner and haven't worked at something open source related at all
and now I've revamped [the book](https://book.veloren.net), redesign the
...
Month in Rust GameDev” where veloren showed up with beautiful screenshots. After
reading some chapters of the book, I then looked for an open source project to
gain practical experience and remembered this promising project.

The sheer size of veloren’s code was intimidating at first, so I was more than
...
with it. To me the code base is clean and most times easy to understand, with
...
Congrats to the Veloren community on the big :100:.
...
Speaking of contributing code, I’ve recently been experimenting with the RTSim
...
ago, to see if the language was mature enough for me to pick it up, as I wanted
...
As time passed by, I moved onto my own projects, but I'm still maintaining the
...
Honestly, I started working on it with literally no programming knowledge
